/**
  ...
  *   - D-Cache: the ring buffer is touched by the CPU only (thread mode writes,
  *     ISR reads) - no DMA is involved - so it needs no MPU carve-out and is
  *     safe with D-Cache enabled (clean self-modifying-free single-core data).
  ******************************************************************************
  */
...
/* ---- newlib retarget: any stray printf() also goes out non-blocking -------
 * NOTE: with newlib-nano + nosys.specs this is the only _write() in the image.
 * The previous ITM/SWO stub in Core/Src/syscalls.c was removed on purpose so
 * that printf() and PRINT_LOG() share one USART1 console.
 * Guarded to GCC/newlib: ARM Compiler (MDK-ARM) retargets printf through
 * _sys_write() instead, and does not link Core/Src/syscalls.c at all. */
