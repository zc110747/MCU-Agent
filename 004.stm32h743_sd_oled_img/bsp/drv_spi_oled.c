...
{  
	...
	{
		sprintf( Number_Buffer , "%0*ld", (int)len, (long)number );
	}
	else			// 多余位补空格
	{	
		sprintf( Number_Buffer , "%*ld", (int)len, (long)number );	// 将 number 转换成字符串，便于显示		
	}
...
{  
	while(*pText != 0)
	{
		...
		{
			/* CH font table not loaded: skip to avoid NULL deref */
			if (LCD_CHFonts == NULL) { pText += 2; continue; }
