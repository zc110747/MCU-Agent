/**
 * @file    main.c
 * @brief   Board bring-up for the face-detection demo.
 *
 * Clock tree (25 MHz HSE crystal on the LXB743ZI-P1):
...
int main(void)
{
    ...
    if (app_face_init() != RT_OK)
    {
        /* The application already reported the reason on the panel and the
         * console; blink the LED so the board is obviously not running. */
        for (;;)
        {
            LED_TOGGLE();
            HAL_Delay(200);
 ...
 * 256 KB back out as *non-cacheable*: that is where the DCMI DMA writes the
