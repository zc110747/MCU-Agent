...
HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority)
{
  ...
  /* TIM7 is on APB1: timer clock = 2 * PCLK1 when APB1 prescaler > 1 */
  ...
  /* Prescaler to get a 1 MHz counter clock */
  ...
  if (status == HAL_OK)
  {
    /* HAL_RCC_ClockConfig() re-calls HAL_InitTick() with the new clocks;
       ...
       Start_IT would fail, silently killing the 1 ms time base (observed:
       uwTick froze, HAL_Delay() hung).  Stop first to reset State=READY. */
