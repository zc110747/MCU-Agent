...
/**
  * @brief  Fonts are loaded from a volume; now bring up the audio path and
  *         switch to Screen 2 (the music player).  Does not auto-play: the
  *         user presses 播放 on the player UI (per spec).
  */
static void enter_player(const char *vol)
{
    app_ui_show_centered("正在加载音乐...");
    lv_timer_handler();   /* flush the "loading" text before the slow scan */

    if (player_init() != 0)
    {
        PRINT_LOG("[UI  ] audio device init FAILED\r\n");
        app_ui_show_centered("音频设备初始化失败!");
        s_state = LOAD_FAILED;
        return;
    }
    ...
    music_ui_create();
    s_state = LOAD_OK;
    PRINT_LOG("[UI  ] music player screen up (vol=%s)\r\n", vol);
}

/* -------------------------------------------------------------------------- */
/* Lightweight serial test console                                            */
/* -------------------------------------------------------------------------- */
/* Lets a PC terminal drive the player without touching the touch screen, so
