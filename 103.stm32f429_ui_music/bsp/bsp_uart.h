...
/**
  * @brief  Non-blocking RX read.  Returns 1 and stores the byte if one is
  *         waiting, 0 otherwise.  Polled (no RX interrupt needed), safe from
  *         any task.  The UI task uses it as a lightweight test console to
  *         drive play/prev/next/volume without touching the screen.
  */
