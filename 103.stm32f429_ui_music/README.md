# STM32F429IGT6 音乐播放器（UI + WM8978 + SAI + FatFs exFAT + FreeRTOS）

> STM32F429IGT6 通过 **USB FS Host (TinyUSB)** 读取 U 盘 / microSD（MSC → SCSI → FatFs），
> 支持 **真正的 exFAT 文件系统**（非 FAT32 伪装），FreeRTOS 堆置于外部 SDRAM；
> **WM8978 编解码器**经 **SAI1（I2S 主发）**播放 `.wav` / `.mp3` 音乐，LVGL 深色 UI
> 显示曲目信息 / 进度 / 音量。调试串口为 **USART3 (PB10/PB11)**，运行日志经该串口输出。
...
## 8. 音乐播放器（WM8978 + SAI1 + minimp3）

> 本工程在通用 UI 框架（页 0–3）之上叠加一套 **音乐播放器**：启动加载屏等待 U 盘 / SD
> 就绪后进入音乐播放屏（屏 2，由 `music_ui.c` 实现），通过 WM8978 经 SAI1 播放
> `music/` 目录下的 `.wav` / `.mp3`。

### 8.1 硬件引脚（WM8978 / SAI / I2C / PWM）

| 功能 | 外设 | 引脚 | 备注 |
|------|------|------|------|
| SAI1_FS_A（帧同步 LRCK） | SAI1 Block A | PE4 | I2S 主发，AF6 |
| SAI1_SCK_A（位时钟 BCLK） | SAI1 Block A | PE5 | I2S 主发，AF6 |
| SAI1_SD_A（数据） | SAI1 Block A | PE6 | I2S 主发，AF6 |
| SAI1_MCK_A（主时钟 MCLK） | SAI1 Block A | PE2 | I2S 主发，AF6 |
| SAI1_SDB（未用，悬空） | SAI1 Block B | PE3 | 本工程只用 Block A |
...
`audio_decoder.c` 对上层提供**格式无关**的 PCM 流，统一归一化为 **16-bit 有符号、立体声、L/R 交错**——正好匹配 SAI DMA 半字需求：
...
`audio_player.c` 在 `player_init()` 里 `wm8978_init()` → `sai_audio_init(44100,2,16)` → `wm8978_set_volume(70)` → 起 `audio` 任务（优先级 idle+1，栈 2048）。对外 API：

`player_scan(vol)`（扫 `vol/music`，按扩展名 `.wav/.mp3` 过滤，最多 `PLAYER_MAX_TRACKS=64`）→
`player_load(idx,autoplay)`（开解码器 + `sai_audio_configure(sr)` + 起播/暂停）→
`player_task()`（等 `sai_audio_get_empty_half()` 空半区信号量，解码填入，EOF 调 `player_advance_auto()`）→
`player_play/pause/toggle/stop/next/prev/seek_percent/set_volume/get_volume/current_index/current_title/position_ms/duration_ms/sample_rate`。

- **next/prev 保持播放状态**：切曲时若当前在播放则新曲自动续播，停止则保持停止（符合需求“保留当前播放/停止状态”）。
- **点击死机修复（用户实测项）**：原 `s_dec`/`s_state` 在 `ui_task`（按钮回调）与 `audio` 任务间**无互斥**——播放中切歌会 `audio_decoder_close()`+清零 `s_dec`，而 `audio` 任务可能正处 `audio_decoder_read` 使用已清零的 `s_dec->fil`/`s_dec->mp3` → NULL 解引用 → HardFault → 整板死机。修复：加 `s_lock`（`xSemaphoreCreateRecursiveMutex` 递归互斥量），所有读写 `s_dec`/`s_state` 的 API（`play/pause/toggle/stop/next/prev/seek/load`）统一 `PLOCK_TAKE/PLOCK_GIVE`；`player_toggle` 内嵌 `play/pause` 靠递归互斥量允许嵌套；`player_task` 先**无锁**等空半区信号量，拿锁后再复核 `PLAYING/opened` 才解码，避免 UI 命令中途清零上下文（见 §8.6）。
- **music_ui.c（屏 2）**：标题 + `序号/总数` + **可拖拽进度条**（点击/按住轨道 → `lv_indev_get_act()` 取点 → `player_seek_percent`）+ 传输行 `[上一首/播放/下一首]`（播放键兼暂停切换）+ 右侧**竖向音量条 + 音量 ± 按钮**（带数字）。10 Hz `lv_timer` 刷新进度/音量。
- **LVGL 约束**：本工程 `lv_slider` 关闭，进度条/音量条用 **`lv_bar` + 可点击轨道 + 句柄对象**自绘（见 §3.4 深色极简约定）。
...
> 软件侧（双构建零警告、状态机、解码归一化、UI 接线）已全部落地；**音频时钟精度与真机出声**依赖用户在本板用示波器/耳机标定（§8.2），属交付后硬件验证项，不阻塞固件构建。

### 8.6 本轮修复记录（2026-08-29，用户实测后反馈）
...
3. **中文曲名乱码（已修复）**
   - 根因：`third_party/FatFs/ffconf.h` 原 `FF_LFN_UNICODE=0` 让 `f_readdir` 把长文件名按 GBK（代码页 936）字节返回，而 `lv_font_gbk` 接收的是 LVGL 从 **UTF-8** 文本解出的 Unicode 码点，GBK 字节被当 UTF-8 误解码 → 中文乱码/空白。
...
│   ├── ui_task.c/.h               # ★ 启动加载状态机 + 进入音乐播放器（player_init/scan）+ LVGL 渲染泵
│   ├── app_ui.c/.h                # ★ UI 框架：布局 / 导航条 / 2 Hz tick / 页面分发 / 公共 API
│   ├── music_ui.c/.h              # ★ 音乐播放器界面（屏 2：曲名/序号/进度条可拖拽/音量 ±）
│   ├── audio_decoder.c/.h         # ★ 格式无关 PCM 解码：WAV(PCM 8/16) + MP3(minimp3) → 16bit 立体声交错
│   ├── audio_player.c/.h          # ★ 播放引擎：scan/init/play/pause/next/prev/seek/volume 状态机 + audio 任务
...
│   ├── bsp_wm8978.c/.h           # ★ WM8978 编解码器：I2C 初始化 + 音量(0..100) + DAC→HP/SPK 路由
│   ├── bsp_sai_audio.c/.h        # ★ SAI1 Block A 主发 (I2S) + DMA 双缓冲 + PLLSAI 时钟 + 空半区信号量
│   └── bsp_ap3216 / bsp_mpu9250 / bsp_eeprom_24c02 / bsp_delay
├── Drivers/                        # CMSIS + STM32F4 HAL（含 stm32f4xx_hal_sai.c / _sai_ex.c）
├── third_party/
│   ├── FreeRTOS-Kernel/           # V11，heap_5
│   ├── FatFs/                     # R0.15，FF_FS_EXFAT=1，FF_CODE_PAGE=936(GBK 中文曲名)
│   ├── tinyusb/                   # 0.21.0 主机栈
│   ├── lvgl/                      # v8，深色极简主题（lv_slider 关闭，进度条用 lv_bar + 点击轨道）
│   └── audio/minimp3/             # CC0 MP3 解码器（帧 API：mp3dec_t + mp3dec_decode_frame）
