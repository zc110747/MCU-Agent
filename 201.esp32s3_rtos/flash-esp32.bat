...
REM ============================================================
REM  flash-esp32.bat - direct flash + reset for ESP32-S3
REM
REM  This script does NOT compile. Build the firmware externally
REM  (e.g. build.bat / arduino-cli compile) so that the .build
REM  directory exists before running this.
REM
...
if not exist "%BUILD%" (
    ...
    echo       Compile the firmware first (e.g. build.bat^), then re-run this script.
    %PAUSECMD%
    exit /b 1
)
...
echo.
set "CNT=0"
REM Use Python (pyserial) to enumerate ports: WMI/.NET are unreliable
REM on some systems and miss USB-serial (e.g. CH343) adapters.
set "PY="
