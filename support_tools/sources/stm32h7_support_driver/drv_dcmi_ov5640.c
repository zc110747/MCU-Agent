...
size_t init_config_size = sizeof(OV5640_INIT_Config)/4; 
...
size_t af_firmwware_size = sizeof(OV5640_AF_Firmware);
...
GlobalType_t drv_dcmi_ov5640_init(void)
{
    ...
    if (dcmi_ov5640_drop(Display_Width, Display_Height, OV5640_Width, OV5640_Height) != RT_OK)
    {
        return RT_FAIL;
    }
    
    drv_dcmi_trigger_constant();
...
static GlobalType_t dcmi_ov5640_drop(uint16_t display_x, uint16_t display_y, uint16_t sensor_x,uint16_t sensor_y)
{
	...
	if ((display_x>=sensor_x)
    || (display_y>=sensor_y))
	{
		return RT_FAIL;
	}

	x_offset = sensor_x - display_x;
	y_offset = (sensor_y - display_y)/2-1;

	capcnt = display_x*2-1;
	
	vline = display_y-1;
	
	HAL_DCMI_ConfigCrop (&hdcmi, x_offset, y_offset,capcnt,vline);
