...
GlobalType_t driver_spi_oled_init(void)
{
    ...
    // 以下进行一些驱动的默认设置
    OLED_SetDirection(Direction_V);  	   // 设置显示方向
    OLED_SetBackColor(LCD_BLACK);           // 设置背景色
    OLED_SetColor(LCD_WHITE);               // 设置画笔色  
    OLED_Clear();                           // 清屏

    OLED_SetAsciiFont(&ASCII_Font24);       // 设置默认字体
    OLED_ShowNumMode(Fill_Zero);	      	   // 设置变量显示模式，多余位填充空格还是填充0
