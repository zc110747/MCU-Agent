...
/* UART HAL header references DMA_HandleTypeDef, so DMA must be enabled. */
...
/* QSPI HAL header references MDMA_HandleTypeDef, so MDMA must be enabled. */
...
/**
  ...
  */
/* This project sources the system PLL from the board's 25 MHz HSE crystal,
 * so HSE_VALUE must match the crystal actually fitted (25 MHz passive xtal). */
