...
void touch_task(void *arg)
{
    ...
    printf("[TOUCH] task started, probing GT9147 (T_SCK=PH6 T_MOSI=PI3 "
           "T_CS=PI8 T_PEN=PH7)\r\n");

    if (bsp_touch_init() != 0)
    {
        printf("[TOUCH] disabled: controller not answering\r\n");
        for (;;)
        {
    ...
    printf("[TOUCH] INT armed on T_PEN (PH7, falling edge) -> waiting\r\n");
    ...
    for (;;)
    {
        ...
        if ((signalled != 0) && (wake_logged == 0U))
        {
            ...
            wake_logged = 1U;
            printf("[TOUCH] T_PEN interrupt received (irq=%lu)\r\n",
        ...
        while (idle_polls < TOUCH_RELEASE_POLLS)
        {
            if (bsp_touch_scan() > 0)
            {
                ...
                if ((signalled == 0) && (bsp_touch_irq_count() == irq_before))
                {
                    ...
                    printf("[TOUCH] WARNING: contact seen without T_PEN "
