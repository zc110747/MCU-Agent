...
提示词内容：[项目生成时提示词](./007.stm32h743_cmsis_dap/prompter.md)

## 使用AI Agent开发的项目的经验总结

主要体验是开发方便，在提供需要的调试工具后，只需要提示词就可以生成项目，不需要手动配置环境。

对于AI Agent项目也踩了很多的坑，从提示词到实践，需要清晰的认知，才能正常的工作。

初步总结下经验:

1. 在提需求时，要对项目有清晰的认知，包括功能需求、硬件信息、开发步骤和验证手段
2. 对于简单的驱动，如rtc、gpio、i2c内部模块完全没有难度，但是对于外部器件(如LCD、CMOS等)，如果直接成功还可以，不成功则重复调试很久都不会成功；调试过的硬件，提供源码比直接生成稳定的多，也更容易开发
3. 一定要提供完整的单片机开发仿真环境，实现代码才是第一步，无法验证的代码生成后，再去开发调试还要去理解，花费精力并不少

AI Agent对于开发最大的影响是对于掌握开发思路，但是开发时间成本高、知识点多的项目，提供了去实践的机会。

以上面的人脸检测项目为例，从原理上包含图像采集、预处理、模型推理、图像显示合并、显示输出等步骤。

这个需求让我实现，图像采集、预处理、显示输出只是工程量的问题，需要花时间就可以；模型推理部分，需要图像库整理、模型训练、模型部署等步骤，对我来说，去学习掌握完整流程和实践可能要几周或者几个月，Vibe Coding降低了这个门槛，只需要两个小时就实现了人脸检测功能。

在本系列中，将使用AI Agent进行单片机项目的开发，验证边界和最佳实践。
