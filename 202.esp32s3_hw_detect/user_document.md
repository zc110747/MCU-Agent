...
默认输入上拉，状态实时刷新；点 **SET / CLEAR** 经 `gpio_set` 提升为输出并驱动，方向掩码写入 NVS 重启保持。

### 5.3.2 WS2812 状态灯（GPIO48）
板载 WS2812B 运行灯，支持 5 种状态：**关闭 / R 闪烁 / G 闪烁 / B 闪烁 / RGB 循环**。
点对应按钮即下发 `ws2812_set`，当前模式高亮显示。亮度由 `LED_BRIGHTNESS=40` 固定（防刺眼）。
...
驱动方式为 ESP32 内置 LEDC（10-bit 分辨率）。引脚经 PinManager 占用，冲突会被拒绝（页面提示后保持原状）。
典型用途：舵机 / 蜂鸣器 / LED 调光 / 电机使能。
...
{ "type":"led",     "gpio":48, "mode":0, "timestamp":... }   // mode: 0=off 1=r 2=g 3=b 4=cycle
{ "type":"pwm",     "pin":21, "period":1000, "duty":50, "active":1 }
...
| `verify_ws.py` | WS `:81` 握手 101、持续接收带 type 的 JSON、心跳观测 |
| `run_all.py` | 汇总以上 4 项，输出 `TOTAL: n/4 scripts passed`，退出码 0/1 |
...
- **WS2812 亮度**：固件内 `LED_BRIGHTNESS=40`，偏暗防刺眼；如需更亮改 `debug/ws2812_led.cpp` 后重编。
